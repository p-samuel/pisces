program ModelState;

uses
  Pisces.Attributes in '..\..\src\Pisces.Attributes.pas',
  Pisces.Base in '..\..\src\Pisces.Base.pas',
  Pisces.EventListeners in '..\..\src\Pisces.EventListeners.pas',
  Pisces.Logger in '..\..\src\Pisces.Logger.pas',
  Pisces in '..\..\src\Pisces.pas',
  Pisces.Registry in '..\..\src\Pisces.Registry.pas',
  Pisces.Types in '..\..\src\Pisces.Types.pas',
  Pisces.Utils in '..\..\src\Pisces.Utils.pas',
  Pisces.View in '..\..\src\Pisces.View.pas',
  Pisces.ViewGroup in '..\..\src\Pisces.ViewGroup.pas',
  Pisces.Lifecycle in '..\..\src\Pisces.Lifecycle.pas',
  Pisces.App in '..\..\src\Pisces.App.pas',
  Pisces.ScreenManager in '..\..\src\Pisces.ScreenManager.pas',
  Pisces.Audio in '..\..\src\Pisces.Audio.pas',
  Pisces.Keyboard in '..\..\src\Pisces.Keyboard.pas',
  Pisces.JNI.Extensions in '..\..\src\Pisces.JNI.Extensions.pas',
  Pisces.Adapters in '..\..\src\Pisces.Adapters.pas',
  Pisces.State in '..\..\src\Pisces.State.pas',
  Pisces.State.Model in '..\..\src\Pisces.State.Model.pas',
  Model.TripPlanner in 'src\Model.TripPlanner.pas',
  View.EditTrip in 'src\View.EditTrip.pas',
  View.EditTraveler in 'src\View.EditTraveler.pas',
  View.EditSettings in 'src\View.EditSettings.pas',
  View.EditNotes in 'src\View.EditNotes.pas',
  View.EditItinerary in 'src\View.EditItinerary.pas',
  View.EditActivity in 'src\View.EditActivity.pas',
  View.EditReminders in 'src\View.EditReminders.pas',
  View.Main in 'src\View.Main.pas';

begin

end.
